...
const encodings$2 = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789$_';
...
for( var x$1 = 0; x$1 < 256; x$1++ ) {
	if( x$1 < 64 ) {
		decodings$1[encodings$2[x$1]] = x$1;
	}
}
...
function base64ArrayBuffer(arrayBuffer) {
  ...
  for (var i = 0; i < mainLength; i = i + 3) {
    ...
    base64 += encodings$2[a] + encodings$2[b] + encodings$2[c] + encodings$2[d];
  }
  ...
  if (byteRemainder == 1) {
    ...
    b = (chunk & 3)   << 4; // 3   = 2^2 - 1
    base64 += encodings$2[a] + encodings$2[b] + '==';
    ...
    base64 += encodings$2[a] + encodings$2[b] + encodings$2[c] + '=';
  }
...
function DecodeBase64$1( out, buf )
{
	var n;
	var outsize = 0;
	var l = (buf.length+3)/4;
...
for( var a = 0; a < encodings$2.length; a++  ) {
   var r = (xor_code_encodings[encodings$2[a]]={} );
   for( var b = 0; b < encodings$2.length; b++  ) {
	r[encodings$2[b]] = encodings$2[a^b];
   }
}
xor_code_encodings['='] = {'=': '='};
...
function makeXKey( key, step ) {
	...
	, setKey(key,step) { this.key = key; this.keybuf = DecodeBase64$1( new Uint8Array(Math.ceil(key.length*3/4)), key); this.step = step?step:0; } };
}
...
const JSOX$1 = exports$1;
JSOX$1.version = "1.2.105";
				...
				case VALUE_ARRAY:
					...
					if( arrayType >= 0 ) {
						...
						if( val.contains.length )
							ab = DecodeBase64$2( val.contains[0] );
						else ab = DecodeBase64$2( val.string );
...
JSOX$1.parse = function( msg, reviver ) {
	...
	if( _parser.length <= parse_level )
		_parser.push( Object.freeze( JSOX$1.begin() ) );
...
/* init prototypes */
{
	...
	toProtoTypes.set( String.prototype, { external:false
	    , name : "String"
	    , cb:function(){ return '"' + JSOX$1.escape(this_value.apply(this)) + '"' } } );
	...
	toProtoTypes.set( ArrayBuffer.prototype, { external:true, name:"ab"
	    , cb:function() { return "["+base64ArrayBuffer$1(this)+"]" }
	} );

	toProtoTypes.set( Uint8Array.prototype, { external:true, name:"u8"
	    , cb:function() { return "["+base64ArrayBuffer$1(this.buffer)+"]" }
	} );
...
JSOX$1.registerFromJSOX = function( prototypeName, o /*, f*/ ) {
...
JSOX$1.registerToFrom = function( prototypeName, prototype/*, to, from*/ ) {
...
JSOX$1.stringifier = function() {
	...
	for( var x$2 = 0; x$2 < 256; x$2++ ) {
		if( x$2 < 64 ) {
			decodings$2[encodings$3[x$2]] = x$2;
		}
	}
	Object.freeze( decodings$2 );
	...
	function base64ArrayBuffer$1(arrayBuffer) {
		...
		for (var i = 0; i < mainLength; i = i + 3) {
			...
			// Convert the raw binary segments to the appropriate ASCII encoding
			base64 += encodings$3[a] + encodings$3[b] + encodings$3[c] + encodings$3[d];
		}
	...
	// Deal with the remaining bytes and padding
		if (byteRemainder == 1) {
			...
			// Set the 4 least significant bits to zero
			b = (chunk & 3)   << 4; // 3   = 2^2 - 1
			base64 += encodings$3[a] + encodings$3[b] + '==';
		} else if (byteRemainder == 2) {
			chunk = (bytes[mainLength] << 8) | bytes[mainLength + 1];
			a = (chunk & 64512) >> 10; // 64512 = (2^6 - 1) << 10
			b = (chunk & 1008)  >>  4; // 1008  = (2^6 - 1) << 4
			// Set the 2 least significant bits to zero
			c = (chunk & 15)    <<  2; // 15    = 2^4 - 1
			base64 += encodings$3[a] + encodings$3[b] + encodings$3[c] + '=';
		}
		//console.log( "dup?", base64)
		return base64
	}
	...
	function DecodeBase64$2( buf ) {	
		...
		else if( decodings$2[buf[buf.length - 3]] == -1 )
			outsize = ((((buf.length + 3) / 4)|0) * 3) - 3;
		else if( decodings$2[buf[buf.length - 2]] == -1 ) 
			outsize = ((((buf.length + 3) / 4)|0) * 3) - 2;
		else if( decodings$2[buf[buf.length - 1]] == -1 ) 
			outsize = ((((buf.length + 3) / 4)|0) * 3) - 1;
		else
		...
		for( n = 0; n < l; n++ ) {
			var index0 = decodings$2[buf[n*4]];
			var index1 = (n*4+1)<buf.length?decodings$2[buf[n*4+1]]:-1;
			var index2 = (index1>=0) && (n*4+2)<buf.length?decodings$2[buf[n*4+2]]:-1 ;
			var index3 = (index2>=0) && (n*4+3)<buf.length?decodings$2[buf[n*4+3]]:-1 ;
			if( index1 >= 0 )
				out[n*3+0] = (( index0 ) << 2 | ( index1 ) >> 4);
			if( index2 >= 0 )
				out[n*3+1] = (( index1 ) << 4 | ( ( ( index2 ) >> 2 ) & 0x0f ));
