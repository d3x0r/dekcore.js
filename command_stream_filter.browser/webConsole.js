...
class createSpawnServer extends stream.Duplex {
constructor( sandbox ) {
           super();
	...
	server.onaccept( async function (conn) {
		sack.log( "Connection received with : ", conn.headers['Sec-WebSocket-Protocol'], " path:", conn.url) ;
		...
		create( await name+":"+counter++, await description).then( (e)=>{
			...
			e.wake().then( ()=>{
				...
				if( conn.url.endsWith( "/EntityProxy" ) )
					e.require(  "./startupWebRemote.js" ).then( ()=>{
						conn.post( e.Λ );
					})
				else
