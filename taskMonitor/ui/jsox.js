		...
		_write(msg,complete_at_end) {
			...
			function convertValue() {
				...
				case VALUE_EMPTY:
					return undefined;
				case VALUE_OBJECT:
					if( val.className ) { fp = fromProtoTypes.get( val.className ); val.className = null; if( fp ) return fp.call( val.contains ); }
					return val.contains;
				case VALUE_ARRAY:
					if( arrayType >= 0 ) {
			...
			function openObject() {
				...
				if( parse_context == CONTEXT_UNKNOWN ) {
					if( word == WORD_POS_FIELD /*|| word == WORD_POS_AFTER_FIELD*/ && val.string.length ) {
						if( fromProtoTypes.get( val.string ) ) {
...
JSOX.stringifier = function() {
	...
	function stringify( object, replacer, space ) {
		...
		// Produce a string from holder[key].
			...
			var protoConverter = (value !== undefined && value !== null) && ( toProtoTypes.get( Object.getPrototypeOf( value ) ) || null )
			var objectConverter = (value !== undefined && value !== null) && ( toObjectTypes.get( Object.keys( value ).toString() ) || null )

		console.log( "TEST()", value, protoConverter, objectConverter );

			var toJSOX = ( protoConverter && protoConverter.cb ) || ( objectConverter && objectConverter.cb );
